...
const manifestPath = path.join(
  projectRoot,
  "client/src/content/blog/posts.json",
);
...
describe("blog content", () => {
  ...
  it("keeps the required application, access, and personal-advice sections", async () => {
    ...
    expect(article).toContain(
      "Shriram Krishnamurthi’s [advice to recommendation writers]",
    );
    expect(article).toContain("## Acknowledgments");
    expect(article).toContain(
      "I am grateful to Po-Chen Ko for his thoughtful feedback",
    );
    expect(article).toContain("Queer in AI");
    expect(article).toContain("discussing research with him");
    expect(article).toContain(
      "[CS PhD Application Coach](https://10-oasis-01.github.io/cs-phd-application-coach/)",
    );
